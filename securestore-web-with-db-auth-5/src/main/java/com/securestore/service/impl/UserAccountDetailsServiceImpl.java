...
public class UserAccountDetailsServiceImpl implements UserAccountDetailsService {
	...
	public UserDetails loadUserByUsernameAndDomain(String username, String domain) throws UsernameNotFoundException {
		logger.debug("loadUserByUsernameAndDomain called");
		UserAccount user = userRepository.findByUsernameAndDomain(username, domain);
		logger.debug("retrieved userAccount: " + user);
		if (user == null) {
